...
Yes, but in a different way. When you send: 10,000 input tokens. the model first processes the entire input to build its internal representation. After that, it begins generating output one token at a time.

So the total time is approximately: Time to process the input + Time to generate each output token
